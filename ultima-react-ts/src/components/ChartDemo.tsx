...
const getLineData = (colorMode: string) => {
...
const getBarData = (colorMode: string) => {
    const isLight = colorMode === 'light';
    const dataset1Color = isLight ? '#00ACC1' : '#4DD0E1';
    const dataset2Color = isLight ? '#FF9800' : '#FFB74D';
...
const getPolarData = (colorMode: string) => {
    const { limeColor, blueColor, tealColor, orangeColor } = getColors(colorMode);
    const borderColor = getComputedStyle(document.body).getPropertyValue('--divider-color') || 'rgba(160, 167, 181, .3)';
...
let pieChartOptions: object;
let polarChartOptions: object;
let barChartOptions: object;
let radarChartOptions: object;


const ChartDemo = (props: any) => {
    const [lineData, setLineData] = useState<any>(getLineData(props.colorMode));
    const [barData, setBarData] = useState<any>(getLineData(props.colorMode));
    const [polarData, setPolarData] = useState<any>(getLineData(props.colorMode));
    const [pieData, setPieData] = useState<any>(getLineData(props.colorMode));
